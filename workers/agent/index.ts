...
export class EmailAgent extends AIChatAgent<any> {
	...
	async onChatMessage(onFinish: any) {
		...
		return result.toUIMessageStreamResponse({ onError: (error) => error instanceof Error && error.name === "AIBudgetError" ? error.message : "AI request failed." });
	}
	...
	async handleNewEmail(emailData: {
		...
		try {
			...
			if (email?.body) {
				const isInjection = await isPromptInjection(createBudgetedAI(env), email.body);
				if (isInjection) {
		...
		if (threadEmails.length > 1) {
			...
			if (threadContext) {
				const threadInjection = await isPromptInjection(createBudgetedAI(env), threadContext);
				if (threadInjection) {
...
Based on the email content and thread context above, draft a reply using draft_reply. If you need more context, use get_thread with thread ID "${emailData.threadId}".`;
		...
		try {
			...
			if (!draftToolCalled && result.text.trim()) {
				...
				const sanitizedText = await verifyDraft(createBudgetedAI(env), result.text.trim());
				if (!sanitizedText) {
